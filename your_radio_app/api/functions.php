		...
		if (isset($_GET['api_key'])) {
			...
			if ($access_key_received == $api_key) {
				...
				if ($this->get_request_method() != "GET") $this->response('',406);
					...
					$sql_featured = "SELECT n.id AS 'radio_id', n.radio_name, n.radio_image, n.radio_url, c.category_name FROM tbl_category c, tbl_radio n WHERE c.cid = n.category_id AND n.featured = 1 ORDER BY n.last_update DESC LIMIT $limit";

					$sql_categories = "SELECT DISTINCT c.cid, c.category_name, c.category_image, COUNT(DISTINCT r.id) as radio_count FROM tbl_category c LEFT JOIN tbl_radio r ON c.cid = r.category_id WHERE c.featured = 1 GROUP BY c.cid ORDER BY c.last_update DESC LIMIT $limit";

					$sql_recent = "SELECT n.id AS 'radio_id', n.radio_name, n.radio_image, n.radio_url, c.category_name FROM tbl_category c, tbl_radio n WHERE c.cid = n.category_id ORDER BY n.id DESC LIMIT $limit";

					$sql_random = "SELECT n.id AS 'radio_id', n.radio_name, n.radio_image, n.radio_url, c.category_name FROM tbl_category c, tbl_radio n WHERE c.cid = n.category_id ORDER BY RAND() LIMIT $limit";
					...
					$respon = array(
						'status' => 'ok', 'featured' => $featured, 'categories' => $categories, 'recent' => $recent, 'random' => $random
					);
					$this->response($this->json($respon), 200);
